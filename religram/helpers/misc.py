...
async def delete_chat_history(app, message):
	return await app.send(raw.functions.messages.DeleteHistory(peer=await app.resolve_peer(message.chat.id), max_id=99999, revoke=True))
